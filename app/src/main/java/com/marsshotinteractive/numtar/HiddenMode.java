...
public class HiddenMode extends AppCompatActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        numGoalTextView.setText("");
        toolbarModeTextView.setText("Hidden Mode");
        generateGoalNumber();
        ...
        countDownTimer = new CountDownTimer(60000, 1000) {
            ...
            public void onFinish() {
                ...
                builder.setTitle("Time is up!")
                        .setMessage("GoalNumber: " + goalNumber)
                        .setCancelable(false)
    ...
    private void generateGoalNumber() {
        ...
        Log.d(DEBUG_TAG, "-------Goal: " + goalNumber + " -----------------");
        lastRange = Math.abs(goalNumber - (currentTotal));
    }
    ...
    //determine the range from the total to the goal number and give feedback to the user
    private void calcRangeFromTotalToGoal()
    {
        int currentRange = Math.abs(goalNumber - (currentTotal));
        Log.d(DEBUG_TAG, "**** Current Range " + currentRange + " *****");
        Log.d(DEBUG_TAG, "**** Last Range " + lastRange + " ****");


        if(goalNumber >= 0)
        {
            Log.d(DEBUG_TAG, "**** >= 0 ****");
            //if(currentRange <= lastRange && currentTotal <= goalNumber)
            if(currentRange <= lastRange)
            {
                numGoalTextView.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), R.color.green_feedback_color));
                //currentRange = lastRange;
                lastRange = currentRange;
                Log.d(DEBUG_TAG, "**** GREEN ****");
            }
            else
            {
                numGoalTextView.setBackgroundColor(ContextCompat.getColor(getApplicationContext(), R.color.red_feedback_color));
                //currentRange = lastRange;
                lastRange = currentRange;
                Log.d(DEBUG_TAG, "**** RED ****");
            }
    ...
    private void endGame()
    {
        ...
        if(currentTotal == goalNumber && timeUp == false)
        {
            ...
            builder.setTitle("You Won!")
                    .setMessage("Goal number was: " + goalNumber)
                    .setCancelable(false)
                    .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int id) {
    ...
    {
        countDownTimer.cancel();
        Intent homeIntent = new Intent(HiddenMode.this, Numtar.class);
